...
- **Sistema "Merci":** Automatización local DevSecOps basada en scripts puros de Python 3.10+ en la carpeta `/scripts/merci/`.
  ...
  - `merci-extract-metrics.py`: Extractor autónomo Data-Driven de métricas (API PageSpeed Insights).
...
10. **Catch-22 de Sincronización Estricta (Zero Trust):** El orquestador `merci-sync-pages.py` utiliza validación estricta (expresiones regulares sin comodines) para sincronizar las estructuras. Si se altera estructuralmente la firma de un bloque principal (por ejemplo, añadiendo un `id` o un atributo WAI-ARIA al `<header>`) en la portada (`index.html`), dicha alteración **debe replicarse manualmente una primera vez** en el resto de los `index.html` estáticos secundarios. De lo contrario, el orquestador no reconocerá los bloques obsoletos y detendrá el pipeline para prevenir inyecciones silenciosas.
