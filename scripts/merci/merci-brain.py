...
def generar_cerebro_estatico(api_key, modelo, force_clean=False):
    ...
    for md_file in biblioteca_dir.rglob("*.md"):
        ...
        prompt = f"Eres un asistente virtual técnico de la web miproyecto.com (Arquitectura DevSecOps). El usuario acaba de entrar a leer el artículo titulado '{titulo}' (Contexto: {desc}). Escribe un saludo directo, inteligente y con un sutil toque 'geek' o de ingeniería (una sola frase, máximo 15 palabras) dándole la bienvenida a este contenido concreto. No uses comillas."
